/* Imob Campeche — mockup behaviour. No framework, no build step. */
(function () {
  'use strict';
  var doc = document;
  ...
  $$('form[data-demo]').forEach(function (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (status) {
        status.hidden = false;
        status.textContent = 'Demonstração: nada foi enviado. No site real, abriria o WhatsApp (48) 99193-8505 com a mensagem: "' + msg + '"';
      }
    });
  });
  ...
  $$('form[data-search]').forEach(function (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      toast('Demonstração: a busca real consultaria o inventário (feed Vista/Loft). Contagens de resultados: [CONFIRM].');
    });
  });
  ...
  /* LGPD cookie banner */
  var KEY = 'imobcampeche_consent_v1';
  var banner = $('#cookie');
