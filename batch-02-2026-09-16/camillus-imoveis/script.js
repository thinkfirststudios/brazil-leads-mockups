/* Camillus Imóveis — mockup behaviour. No framework, no build step. */
(function () {
  'use strict';
  var doc = document;
  ...
  $$('form[data-demo]').forEach(function (form) {
    form.addEventListener('submit', function (e) {
      ...
      if (status) {
        status.hidden = false;
        status.textContent = 'Demonstração: nada foi enviado. No site real, abriria o WhatsApp (48) 99964-3365 (wa.me/5548999643365) com a mensagem: "' + msg + '"';
      }
    });
  });
  ...
  $$('form[data-search]').forEach(function (form) {
    form.addEventListener('submit', function (e) {
      e.preventDefault();
      toast('Demonstração: a busca real consultaria o inventário. Contagem de resultados: [CONFIRM].');
    });
  });
