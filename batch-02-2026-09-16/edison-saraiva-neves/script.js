/* Dr. Edison Saraiva Neves — mockup script (no framework, no build step)
...
(function () {
  ...
  var STORE_KEY = 'esn_cookie_consent_v1';
  ...
  /* ---------- Âncora antiga #depoimentos → Escritos (seção removida por conformidade CFM) ----------
     Fragmentos (#) não chegam ao servidor, então não há 301 possível para eles; o redirecionamento é feito aqui. */
  if (location.hash === '#depoimentos') {
    var target = document.getElementById('escritos');
    if (target) {
      history.replaceState(null, '', '#escritos');
      target.scrollIntoView();
    }
  }

  /* ---------- Mensagem do WhatsApp flutuante conforme a seção visível ---------- */
  var waFloat = document.querySelector('a.wa-float__btn[data-wa-base]');
  var waSections = document.querySelectorAll('[data-wa]');
  if (waFloat && waSections.length && 'IntersectionObserver' in window) {
    var base = waFloat.getAttribute('data-wa-base');
    var defaultHref = waFloat.getAttribute('href');
    var waIo = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          waFloat.setAttribute('href', base + '?text=' + encodeURIComponent(entry.target.getAttribute('data-wa')));
        }
      });
    }, { rootMargin: '-45% 0px -45% 0px' });
    waSections.forEach(function (s) { waIo.observe(s); });
    window.addEventListener('scroll', function () { if (window.scrollY < 200) waFloat.setAttribute('href', defaultHref); }, { passive: true });
  }

  /* ---------- Formulários (demonstração) ---------- */  document.querySelectorAll('form[data-demo]').forEach(function (form) {
