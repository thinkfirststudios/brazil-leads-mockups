/* Imobiliária Novo Campeche — MOCKUP script (no framework, no build step) */
(function () {
  'use strict';
  ...
  var CONSENT_KEY = 'nc_cookie_consent_v1';
  ...
  document.querySelectorAll('.js-demo-form').forEach(function (form) {
    ...
    form.addEventListener('submit', function (e) {
      ...
      var text = 'Olá! Quero anunciar meu imóvel. ' + parts.join(' | ');
      if (WA_NUMBER) {
        window.open(waHref(text), '_blank', 'noopener');
        notice.textContent = 'Abrimos o WhatsApp com a sua mensagem.';
      } else {
        notice.textContent = 'MOCKUP: formulário de demonstração — nada foi enviado. No site real, esta mensagem abrirá no WhatsApp da imobiliária ([CONFIRM número]): "' + text + '"';
      }
      notice.classList.add('is-shown');
    });
  });
