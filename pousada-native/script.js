/* ==========================================================================
   POUSADA NATIVE — spec mockup
   Plain JS. No framework, no build step, no modules, no fetch, no video.

   Performance is part of the argument on this lead: an eco-positioned site
   that ships eight megabytes of autoplaying hero video is an own goal. So
   there is no video, no library, no animation framework — the whole
   behaviour layer is this file, and the texture overlay is CSS, not an
   image request.
   ...
   dialled anywhere in this build — every WhatsApp control is visibly
   disabled and explains why. Set WA_NUMBER once confirmed and the page
   wakes up unchanged.

   ⚠️ NO ENVIRONMENTAL CLAIM IS MADE ANYWHERE IN THIS FILE OR THE PAGES IT
   DRIVES. Not in a message template, not in a placeholder, not in a comment
   presented as fact. The Compromisso list is blanks by design.
   ========================================================================== */
(function () {
  'use strict';
  ...
  /* ------------------------------------------------------- hero parallax --
     Slow and small. The hero is a tight crop of foliage; a big parallax
     throw on a close crop just looks like a glitch. */
    ...
    window.addEventListener('scroll', function () {
      ...
      window.requestAnimationFrame(function () {
        ...
        if (y < window.innerHeight * 1.3) {
          heroImg.style.transform = 'translate3d(0,' + (y * 0.08).toFixed(1) + 'px,0) scale(1.07)';
        }
        ticking = false;
      });
    }, { passive: true });
  ...
  function noNumber() {
    window.alert(t(
      'Nenhum número de WhatsApp foi capturado para a Pousada Native. Não existe site, e o único ' +
      'canal público — o Instagram @nativepousada — fica atrás de uma barreira de login.\n\n' +
      ...
      'mudança.',
      'No WhatsApp number was captured for Pousada Native. There is no website, and the only ' +
      'public channel — the Instagram profile @nativepousada — sits behind a login wall.\n\n' +
  ...
  $$('[data-wa]').forEach(function (el) {
    ...
    var msg = key === 'reserva'
      ? t('Olá! Queria consultar disponibilidade na Pousada Native.',
          'Hello! I would like to check availability at Pousada Native.')
      : t('Olá! Vim pelo site da Pousada Native.',
          'Hello! I came from the Pousada Native website.');
    wireWA(el, msg);
  });

  /* ===================================================================== */
  /*  AVAILABILITY WIDGET                                                   */
  /*  Until a booking engine is chosen this composes a WhatsApp message      */
  /*  with the dates pre-filled. That is a legitimate interim and it is what */
  /*  Brazilian guests expect — but it is stated as what it is, not dressed  */
  /*  up as a live engine. Dates render DD/MM. No rate is quoted, because    */
  /*  none is published anywhere.                                            */
  /* ===================================================================== */
  function ddmm(v) {
  ...
  $$('[data-wa-form]').forEach(function (form) {
    form.addEventListener('submit', function (ev) {
      ...
      var lines = [form.getAttribute('data-wa-title') || 'Pousada Native', ''];
  ...
  /* ------------------------------------------------------- cookie consent -
     LGPD art. 7 / 8. The Instagram embed is a third-party tracker and sits
     behind the marketing category — the static tiles show until marketing
     cookies are accepted. Nothing third-party is loaded in this mockup. */
  var ck = $('#ck');
  var CK_KEY = 'native.consent.v1';
