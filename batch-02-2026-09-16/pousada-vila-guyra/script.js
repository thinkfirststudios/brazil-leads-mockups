/* Pousada Vila Guyrá — mockup behaviour (no framework, no build step).
   WhatsApp number: NOT verified in the brief → body[data-wa] is empty, so no wa.me link is ever generated. */
(function () {
  'use strict';
  ...
  if (heroMedia && !reduce) {
    ...
    var par = function () {
      ...
      if (y < window.innerHeight * 1.2) heroMedia.style.transform = 'translate3d(0,' + (y * 0.22).toFixed(1) + 'px,0)';
      ticking = false;
    };
  ...
  $$('form[data-composer]').forEach(function (form) {
    ...
    inputs.forEach(function (el) {
      el.addEventListener('input', function () { validateField(form, el, false); renderSummary(form); });
      el.addEventListener('change', function () { validateField(form, el, false); renderSummary(form); });
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (WA) {
        var a = doc.createElement('a'); a.className = 'btn btn-salvia btn-sm'; a.href = waLink(msg);
