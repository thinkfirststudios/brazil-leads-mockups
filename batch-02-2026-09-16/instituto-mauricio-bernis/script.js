/* Instituto Mauricio Bernis — mockup behaviour (no framework) */
(function () {
  'use strict';
  var doc = document.documentElement;
  var isEN = (doc.getAttribute('lang') || '').toLowerCase().indexOf('en') === 0;
  var STORE_KEY = 'imb-cookie-consent-v1';
  var reduced = window.matchMedia && window.matchMedia('(prefers-reduced-motion: reduce)').matches;

  var T = isEN ? {
    formOk: 'Demo only — this mockup does not send data. On the live site your message would be delivered to the Instituto and you would receive a reply within [CONFIRM response time].',
    formConsent: 'Please tick the consent box to continue.',
    saved: 'Preferences saved.'
  } : {
    formOk: 'Demonstração — este mockup não envia dados. No site publicado, sua mensagem seria encaminhada ao Instituto, com retorno em [CONFIRM prazo de resposta].',
  ...
  /* Scroll reveals with stagger (no count-up on this build, by brief) */
  ...
  function save(analise, marketing) {
    ...
    closeSheet();
    /* Analytics/marketing tags would only be loaded here, after opt-in. None are loaded in this mockup. */
  }
  if (sheet) {
