/* Açoriana Imóveis — MOCKUP interactions. No framework. */
(function(){
  'use strict';
  ...
  /* §4 BUILD-STAGE SELECTOR — filters the launch grid, counts computed from the cards on the page, live-region announcement */
  var stageBtns = $$('button.stage[data-stage]');
  if(stageBtns.length){
    var devCards = $$('[data-stage-card]');
    var names = {};
    stageBtns.forEach(function(b){
      var s = b.getAttribute('data-stage');
      names[s] = $('.stage-name', b).textContent;
      var n = devCards.filter(function(c){ return c.getAttribute('data-stage-card') === s; }).length;
      var el = $('.count', b); if(el) el.textContent = n + (n === 1 ? ' exemplo' : ' exemplos');
    });
    var stLive = $('#stage-live');
    stageBtns.forEach(function(b){
      b.addEventListener('click', function(){
        var on = b.getAttribute('aria-pressed') !== 'true';
        var s = on ? b.getAttribute('data-stage') : null, n = 0;
        stageBtns.forEach(function(x){ x.setAttribute('aria-pressed', String(on && x === b)); });
        devCards.forEach(function(c){ var show = !s || c.getAttribute('data-stage-card') === s; c.hidden = !show; if(show) n++; });
        if(stLive) stLive.textContent = s ? (n + ' empreendimentos (exemplos) em “' + names[s] + '”') : ('Filtro removido: ' + n + ' empreendimentos (exemplos)');
        var clr = $('#stage-clear'); if(clr) clr.hidden = !s;
      });
    });
    var clrBtn = $('#stage-clear');
    if(clrBtn) clrBtn.addEventListener('click', function(){ var a = $('button.stage[aria-pressed="true"]'); if(a) a.click(); });
    var srail = $('.stage-rail');
    if(srail){
      if(reduce || !('IntersectionObserver' in window)) srail.classList.add('drawn');
      else { var so = new IntersectionObserver(function(en){ if(en[0].isIntersecting){ srail.classList.add('drawn'); so.disconnect(); } }, {threshold:.3}); so.observe(srail.parentNode); }
    }
  }
  ...
  /* LGPD cookie banner */
  var KEY = 'acoriana-cookie-consent-v1';
