/* Hostel Paraíso da Lagoa — mockup behaviour (no framework, no build step).
   WhatsApp: NO verified mobile (only the landline +55 48 3232-1804), so body[data-wa] is empty and no wa.me link is generated.
   The enquiry composer falls back to "call us" with the landline. */
(function () {
  'use strict';
  ...
  var T = {
    pt: {
      ...
      noWa: 'Ainda não há um WhatsApp confirmado [CONFIRM WhatsApp]. Por enquanto, ligue para o hostel e passe as informações acima. Horário de atendimento do telefone: [CONFIRM].',
      call: 'Ligar para +55 48 3232-1804',
    ...
    en: {
      ...
      noWa: 'There is no confirmed WhatsApp number yet [CONFIRM WhatsApp]. For now, please call the hostel and give them the details above. Phone answering hours: [CONFIRM]. Brazilian phone number — international call charges may apply.',
      call: 'Call +55 48 3232-1804',
    ...
    es: {
      ...
      noWa: 'Todavía no hay un número de WhatsApp confirmado [CONFIRM WhatsApp]. Por ahora, llama al hostel y dale los datos de arriba. Horario de atención telefónica: [CONFIRM]. Número brasileño — pueden aplicarse cargos de llamada internacional.',
      call: 'Llamar al +55 48 3232-1804',
  ...
  if (heroMedia && !reduce) {
    ...
    var par = function () {
      ...
      if (y < window.innerHeight * 1.2) heroMedia.style.transform = 'translate3d(0,' + (y * 0.2).toFixed(1) + 'px,0)';
      ticking = false;
    };
  ...
  function validateField(form, el, strict) {
    ...
    if (el.type === 'radio') {
      var any = $$('input[name="' + el.name + '"]', form).some(function (r) { return r.checked; });
      setMsg(el, el.hasAttribute('required') && !any && strict ? t.required : '');
      return !el.hasAttribute('required') || any;
    }
  ...
  function collect(form) {
    ...
    $$('[data-sum]', form).forEach(function (el) {
      ...
      if (v) rows.push([el.getAttribute('data-sum'), el.type === 'radio' ? (el.getAttribute('data-label') || v) : v]);
  ...
  $$('form[data-composer]').forEach(function (form) {
    ...
    var progress = function () {
      var seen = {}, req = inputs.filter(function (el) { if (!el.hasAttribute('required')) return false; if (el.type === 'radio') { if (seen[el.name]) return false; seen[el.name] = 1; } return true; });
      var done = req.filter(function (el) {
        if (el.type === 'radio') return $$('input[name="' + el.name + '"]', form).some(function (r) { return r.checked; });
        var w = fieldWrap(el); return el.value && !(w && w.classList.contains('invalid'));
      }).length;
    ...
    form.addEventListener('submit', function (e) {
      ...
      if (WA) {
        ...
        var n = doc.createElement('p'); n.style.margin = '0 0 10px'; n.style.fontSize = '.95rem'; n.textContent = t.noWa;
        var tel = doc.createElement('a'); tel.className = 'btn btn-lagoa btn-sm'; tel.href = 'tel:+554832321804'; tel.textContent = t.call;
        notice.appendChild(n); notice.appendChild(tel);
      }
      notice.classList.add('show');
